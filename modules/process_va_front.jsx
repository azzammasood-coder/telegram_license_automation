...
function main() {
    ...
    try {
        ...
        if (PATH_VA) {
            ...
            if (laserEdited) {
                ...
                if (r1821) {
                    ...
                    if (g19) {
                        ...
                        if (swirlLayer) {
                            var dlSizes = [3.85, 4.00, 4.05, 3.78, 3.41, 3.22, 2.80, 2.60, 2.60];
                ...
                // Group 11: Lens Image Dob (Forced Multi-line)
                var g11 = getLayerSet(laserEdited, "11 Lens Image Dob");
                if (g11) {
                    var l11 = findArtLayer(g11, "Dob month and full year");
                    if (l11) {
                        l11.textItem.contents = data["Dob Short Month"] + "\r" + data["Dob Year"];
                        l11.textItem.useAutoLeading = true; // Ensures the second line is visible
                        log("Updated Group 11 to Multi-line: " + l11.textItem.contents);
                    }
                }
            ...
            var outDirFront = baseOutputDir + "/Front";
            
            // Create the "Front" subfolder if it doesn't exist
            var fObj = new Folder(outDirFront);
            if (!fObj.exists) fObj.create();
            
            var baseName = data["Base Name"];
            
            log("--- Exporting Layers to: " + outDirFront + " ---");
            
            // 1. [GROUP] 12- 17 RAISED & [GROUP] 18 -21
            if (raised) {
                exportLayer(doc, getLayerSet(raised, "12- 17 RAISED"), outDirFront + "\\12- 17 RAISED.png");
                exportLayer(doc, getLayerSet(raised, "18 -21"), outDirFront + "\\18 -21.png");
            }

            // 2. Laser Edited Groups
            if (laserEdited) {
            ...
            if (laser) {
                var laserNoEdited = getLayerSet(laser, "Laser No Edited");
                if (laserNoEdited) {
                    exportLayer(doc, getLayerSet(laserNoEdited, "1 Laser Light Text DO Not Touch"), outDirFront + "\\1 Laser Light Text DO Not Touch.png");
                }
            }
